...
describe("general", function()
  ...
  local function get_provider_metafile(name)
    return path.join(conf.get_config().cache.dir, "provider_metafile_" .. name)
  end
  ...
  describe("[ProviderSwitch:new]", function()
    it("creates single plain provider", function()
      ...
      if not github_actions then
        local meta1 = utils.readfile(get_provider_metafile("single_test"))
        local result1 = utils.readfile(get_provider_resultfile("single_test"))
    ...
    it("creates single plain_list provider", function()
      ...
      if not github_actions then
        local meta2 =
          utils.readfile(get_provider_metafile("single_plain_list_test"))
        local result2 =
          utils.readfile(get_provider_resultfile("single_plain_list_test"))
    ...
    it("creates multiple plain providers", function()
      ...
      if not github_actions then
        local meta3 = utils.readfile(get_provider_metafile("multiple_test"))
        local result3 = utils.readfile(get_provider_resultfile("multiple_test"))
      ...
      if not github_actions then
        local meta4 = utils.readfile(get_provider_metafile("multiple_test"))
        local result4 = utils.readfile(get_provider_resultfile("multiple_test"))
  ...
  describe("[make_cache_filename]", function()
    it("is debug mode", function()
      vim.env._FZFX_NVIM_DEBUG_ENABLE = 1
      assert_eq(
        general.make_cache_filename("provider", "switch", "meta", "live_grep"),
