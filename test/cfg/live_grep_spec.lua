...
describe("cfg.live_grep", function()
  ...
  describe("[_is_unrestricted_mode/_is_buffer_mode]", function()
    it("_is_unrestricted_mode", function()
      assert_false(live_grep_cfg._is_unrestricted_mode())
      assert_false(live_grep_cfg._is_unrestricted_mode({}))
      assert_false(
        live_grep_cfg._is_unrestricted_mode({ unrestricted = false })
      )
      assert_true(live_grep_cfg._is_unrestricted_mode({ unrestricted = true }))
    end)
    it("_is_buffer_mode", function()
      assert_false(live_grep_cfg._is_buffer_mode())
      assert_false(live_grep_cfg._is_buffer_mode({}))
      assert_false(live_grep_cfg._is_buffer_mode({ buffer = false }))
      assert_true(live_grep_cfg._is_buffer_mode({ buffer = true }))
    end)
  end)
