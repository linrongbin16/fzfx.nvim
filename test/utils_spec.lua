...
describe("utils", function()
  ...
  describe("[FileLineReader]", function()
    ...
    while batch <= 10000 do
      ...
      it(string.format("read lua/fzfx.lua with batch=%d", batch), function()
        local i = 1
  ...
  describe("[parse_flagged_query]", function()
    it("parse without flags1", function()
      local results = utils.parse_flagged_query("asdf")
    ...
    it("parse flags3", function()
      local results = utils.parse_flagged_query("asdf --  -w")
      assert_eq(type(results), "table")
      assert_eq(#results, 2)
      assert_eq(results[1], "asdf")
