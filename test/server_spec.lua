...
describe("server", function()
    ...
    server.setup()
    describe("[_make_uuid]", function()
        it("make", function()
            for i = 1, 5 do
                local actual = server._make_uuid()
                assert_eq(type(actual), "string")
                local actual_splits = utils.string_split(actual, "-")
                assert_eq(type(actual_splits), "table")
                assert_eq(#actual_splits, 4)
                for _, act in ipairs(actual_splits) do
                    assert_true(tonumber(act) >= 0)
                end
            end
        end)
    end)
    describe("[_next_registry_id]", function()
        it("gets bigger", function()
            local id1 = server._next_registry_id()
            assert_true(tonumber(id1) >= 1)
            local id2 = server._next_registry_id()
            assert_eq(tonumber(id2), tonumber(id1) + 1)
            local id3 = server._next_registry_id()
    ...
    describe("[RpcServer]", function()
        ...
        it("register/unregister", function()
            ...
            assert_true(
                tonumber(rid) == tonumber(server._next_registry_id()) - 1
            )
