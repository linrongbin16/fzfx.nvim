...
describe("lib.nvim", function()
  ...
  describe("[buf_is_valid]", function()
    it("valid", function()
      assert_false(nvims.buf_is_valid())
      assert_false(nvims.buf_is_valid(nil))
    end)
  end)
  describe("[get_win_option/set_win_option]", function()
    it("get spell", function()
      nvims.set_win_option(0, "spell", true)
      local s = nvims.get_win_option(0, "spell")
      print(string.format("spell get win option:%s\n", vim.inspect(s)))
      assert_eq(type(s), "boolean")
      assert_true(s)
    end)
    it("set spell", function()
      nvims.set_win_option(0, "spell", false)
      local s = nvims.get_win_option(0, "spell")
  ...
  describe("[ShellOptsContext]", function()
    it("save", function()
      local ctx = nvims.ShellOptsContext:save()
      assert_eq(type(ctx), "table")
      assert_false(tbls.tbl_empty(ctx))
  ...
  describe("[WindowOptsContext]", function()
    it("save", function()
      local ctx = nvims.WindowOptsContext:save()
      assert_eq(type(ctx), "table")
      assert_false(tbls.tbl_empty(ctx))
  ...
  describe("[RingBuffer]", function()
    it("creates", function()
      local rb = nvims.RingBuffer:new(10)
