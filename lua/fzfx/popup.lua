...
local function _make_center_window_config(opts)
  ...
  then
    log.throw("error! invalid option (win_opts.row): %s!", vim.inspect(opts))
  end
  ...
  then
    log.throw("error! invalid option (win_opts.col): %s!", vim.inspect(opts))
  end
...
local function _make_fzf_command(fzf_opts, actions, result)
  ...
  log.debug("|fzfx.popup - _make_fzf_command| command:%s", vim.inspect(command))
  return command
end
...
function Popup:new(win_opts, source, fzf_opts, actions, context, on_popup_exit)
  ...
  local function on_fzf_exit(jobid2, exitcode, event)
    ...
    if exitcode > 1 and (exitcode ~= 130 and exitcode ~= 129) then
      log.err("command '%s' running with exit code %d", fzf_command, exitcode)
      return
    end
