...
function ProviderSwitch:provide(name, query, context)
  ...
  then
    metaopts.prepend_icon_by_ft = provider_config.line_opts.prepend_icon_by_ft
  end
  if
    type(provider_config.line_opts) == "table"
...
local function general(name, query, bang, pipeline_configs, default_pipeline)
  ...
  local provide_rpc_registry_id = server:get_rpc_server():register(provide_rpc)
  local preview_rpc_registry_id = server:get_rpc_server():register(preview_rpc)
  ...
  if type(pipeline_configs.interactions) == "table" then
    for _, interaction_opts in pairs(pipeline_configs.interactions) do
      ...
      table.insert(interaction_rpc_registries, interaction_rpc_registry_id)
...
local function setup(name, pipeline_configs)
  ...
  if schema.is_command_config(pipeline_configs.commands) then
    vim.api.nvim_create_user_command(
      ...
      function(opts)
        local query =
          fzf_helpers.get_command_feed(opts, pipeline_configs.commands.feed)
        return general(
          name,
          query,
          opts.bang,
    ...
    for _, command_configs in pairs(pipeline_configs.commands) do
      vim.api.nvim_create_user_command(command_configs.name, function(opts)
        local query = fzf_helpers.get_command_feed(opts, command_configs.feed)
        return general(
          name,
          query,
          opts.bang,
