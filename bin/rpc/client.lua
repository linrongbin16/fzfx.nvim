...
local SOCKET_MODE = vim.env._FZFX_NVIM_SOCKET_MODE
local SOCKET_ADDRESS = vim.env._FZFX_NVIM_SOCKET_ADDRESS
shell_helpers.log_ensure(
    SOCKET_MODE == "pipe" or SOCKET_MODE == "tcp",
    "|fzfx.bin.rpc.client| error! SOCKET_MODE must be either 'pipe' or 'tcp! %s",
    vim.inspect(SOCKET_MODE)
)
shell_helpers.log_ensure(
...
local channel_id =
    vim.fn.sockconnect(SOCKET_MODE, SOCKET_ADDRESS, { rpc = true })
