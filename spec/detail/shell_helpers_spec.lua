...
describe("detail.shell_helpers", function()
  ...
  describe("[log]", function()
    it("debug", function()
      ...
      assert_true(shell_helpers.log_debug("logs with params 2, %d, %s", 1, "asdf") == nil)
    end)
    it("err", function()
      assert_true(shell_helpers.log_err("logs without params") == nil)
      assert_true(shell_helpers.log_err("logs with params 1, %d", 1) == nil)
      assert_true(shell_helpers.log_err("logs with params 2, %d, %s", 1, "asdf") == nil)
    end)
    it("ensure", function()
      assert_true(shell_helpers.log_ensure(true, "logs without params") == nil)
      local ok, err = pcall(shell_helpers.log_ensure, false, "logs with params 1, %d", 1)
      assert_false(ok)
      assert_eq(type(err), "string")
      assert_true(string.len(err --[[@as string]]) > 0)
    end)
    it("throw", function()
      local ok, err = pcall(shell_helpers.log_throw, "logs with params 1, %d", 1)
      assert_false(ok)
      assert_eq(type(err), "string")
      assert_true(string.len(err --[[@as string]]) > 0)
    end)
