...
- Use `nvim` the lua script interpreter as a RPC client, and the Neovim editor (the one you're editing files) as a RPC server (Special thanks to [@ibhagwan](https://github.com/ibhagwan) and his [fzf-lua](https://github.com/ibhagwan/fzf-lua), I learned this from him). Thus we could send almost every data to the RPC client, and let it handle almost all kinds of logic when generating querying results for fzf.
